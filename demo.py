...
parser = argparse.ArgumentParser(description="Render model with optional slicer mode")

parser.add_argument(
    "--cross_section",
    type=lambda x: str(x).lower() in ["true", "1", "yes", "y"],
    default=True,
    help="Enable cross section view (True/False)"
)

parser.add_argument(
    "--path",
    type=str,
    default="demo_files/enterprise.stl",
...
def render(path="Files/enterprise.stl", cross_section=True):
    ...
    original = get_vectors(path=path)
    ...
    # Transformation Matrices (for plane)
    ...
    while not glfw.window_should_close(window):
        ...
        #rotation
        model = model * glm.mat4_cast(model_ori) # apply quaternion rotation
...
if __name__ == "__main__":
    render(path=model_path, cross_section=args.cross_section)